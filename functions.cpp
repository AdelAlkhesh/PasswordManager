...
void SaveToFile(string path, string email, string password, string accountName)
{
    ...
    if (my_file)
    {
        my_file << setw(12) << std::left << accountName;
        my_file << setw(40) << std::left << email;
        my_file << setw(15) << std::left << password;
        my_file << endl;
    }
    my_file.close();
}
...
void Welcome()
{
    ...
    else
    {
        ...
        if (my_file)
        {
            my_file << setw(12) << std::left << "ACCOUNT NAME";
            my_file << setw(40) << std::left << "EMAIL";
            my_file << setw(15) << std::left << "PASSWORD";
            my_file << endl;
            signUp();
...
{
    const char alphanum[] = "0123456789!@#$%^_-*abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
